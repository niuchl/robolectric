...
public class ShadowServiceTest {
  ...
  public void stopForegroundDoesntDetachNotificationUnlessAsked() {
    service.startForeground(21, notBuilder.build());
    service.stopForeground(false);
    assertThat(shadowOf(service).getLastForegroundNotificationAttached()).isTrue();
  }
  ...
  /**
   * Since Nougat, it's been possible to detach the foreground notification
   * from the service, allowing it to remain after the service dies.
   */
  @Test
  @Config(minSdk = N)
